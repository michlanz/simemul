...
function plannedCampaignPaths(runModes::Vector{Symbol}; root::String = ".")
    planned = NamedTuple[]
...
function runCampaign(runModes::Vector{Symbol}, analysisRunModes::Vector{Symbol})
    ...
    for (modeIdx, runMode) in enumerate(runModes)
        ...
        elseif runMode == :anova
            for (analysisIdx, analysis) in enumerate(analysisPaths)
        ...
        elseif runMode == :evaluation
            for (analysisIdx, analysis) in enumerate(analysisPaths)
                ...
                Main.showevaluation.performEvaluation(input_dir = analysis.path, output_dir = string(analysis.path) * "_evaluation")
            end
        else
